...
def _cfg(tmp_path, cron: str = "*/5 * * * *", tz: str = "UTC") -> str:
    ...
    c.write_text(
        ...
        f"worker: {{global_concurrency: 1, lease_seconds: 3600, timezone: '{tz}'}}\n"
    )
    return str(c)
...
def _t(h: int, m: int) -> datetime:
    ...
    Note: 03:05:30 UTC = 10:05:30 UTC (UTC+7)
    """
...
def test_timezone_conversion_correct(herder_home, tmp_path):
    ...
    cfg = load_config(_cfg(tmp_path, cron="0 10 * * *", tz="Asia/Tokyo"))  # 10:00 local (UTC+9)
    store = Store.open()

    # 01:00 UTC = 10:00 Asia/Tokyo → should match
    n = tick(cfg, store, datetime(2026, 6, 10, 1, 0, 0, tzinfo=timezone.utc))
    assert n == 1

    # 01:01 UTC = 10:01 Asia/Tokyo → should not match
    n = tick(cfg, store, datetime(2026, 6, 10, 1, 1, 0, tzinfo=timezone.utc))
    assert n == 0
