...
def _cfg(tmp_path: Path, *, active: int = 100, perday: int = 500, dedup: bool = True) -> str:
...
def _req(prompt: str = "hello", **kw) -> EnqueueRequest:
    ...
    Returns:
        EnqueueRequest.
    """
    return EnqueueRequest(project="p", role="planner", kind="research", prompt=prompt, **kw)
...
def test_idempotency_key_still_hits_budget(herder_home, tmp_path):
    ...
    with pytest.raises(BudgetError):
        enqueue_job(cfg, store, _req("b", idempotency_key="key2"))  # budget still enforced
