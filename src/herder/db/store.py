...
from herder.transitions import IllegalTransitionError, assert_transition
...
class Store:
    ...
    def count_active_jobs(self) -> int:
        ...
        return self.conn.execute(
            f"SELECT COUNT(*) FROM jobs WHERE status NOT IN ({ph})", self._TERMINAL
        ).fetchone()[0]
    ...
    def count_jobs_since(self, iso_ts: str) -> int:
        ...
        """
        return self.conn.execute(
            "SELECT COUNT(*) FROM jobs WHERE created_at >= ?", (iso_ts,)
        ).fetchone()[0]

    def find_active_duplicate(
        self, role: str | None, project: str | None, kind: str | None, prompt_hash: str
    ) -> sqlite3.Row | None:
        ...
        return self.conn.execute(
            ...
            (prompt_hash, role, project, kind, *self._TERMINAL),
        ).fetchone()
    ...
    def enqueue(self, **fields) -> str:
        ...
        self.conn.execute(f"INSERT INTO jobs ({cols}) VALUES ({placeholders})", fields)
        return fields["id"]
    ...
    def claim_job(self, worker_id: str, lease_seconds: int) -> sqlite3.Row | None:
        ...
        Claimable jobs are those with status='pending'/'approved' whose
        ``next_eligible_at`` is NULL or in the past, or status='running'
        with an expired lease (reclaim).
        ...
        return self.conn.execute(
            """UPDATE jobs
               ...
               RETURNING *""",
            (worker_id, now_iso, lease_iso, now_iso, now_iso, now_iso),
        ).fetchone()
    ...
    def _record_job_event(
        ...
        """Insert one audit row into job_events.

        Args:
            job_id: The job whose status changed.
            from_status: Status before the transition (None if unknown).
            to_status: Status after the transition.
            reason: Short label identifying the caller (e.g. 'finish', 'requeue').
        """
        self.conn.execute(
            "INSERT INTO job_events (job_id, from_status, to_status, reason, at)"
            " VALUES (?,?,?,?,?)",
            (
                job_id,
                from_status,
    ...
    def job_events(self, job_id: str) -> list[sqlite3.Row]:
        """Return all FSM transition events for a job, ordered chronologically.

        Args:
            job_id: The job id.

        Returns:
            List of job_events rows ordered by id (insertion order).
        """
        return self.conn.execute(
    ...
    def finish_job(
        ...
        *,
        ...
        """Mark a job as finished (done, failed, cancelled, or other terminal state).

        Guards the transition via LEGAL_TRANSITIONS before executing the UPDATE.
        Idempotent: a second call with the same status is a silent no-op.
        Atomic: UPDATE WHERE status IN ('running','cancelling') is belt-and-suspenders
        against a concurrent status change between the read and the write.

        Args:
            job_id: The job id.
            status: Target terminal status ('done', 'failed', 'cancelled', …).
            error_type: Optional error type (e.g. 'timeout', 'validation').
            output_path: Optional path to job output/result.

        Raises:
            IllegalTransitionError: If job does not exist, or current → status
                is not a legal FSM transition.
        """
        row = self.get_job(job_id)
        if row is None:
            raise IllegalTransitionError(f"no such job: {job_id}")
        cur = row["status"]
        if cur == status:
            return  # idempotent: already at target
        assert_transition(cur, status, reason="finish")
        ...
        # Only record the audit event if the UPDATE actually transitioned a row. A
        # concurrent writer (e.g. a CLI cancel racing this finish) may have changed the
        # status out of ('running','cancelling') between the read and this write, matching
        # 0 rows — recording an event then would falsify the audit trail.
        if cursor.rowcount == 1:
            self._record_job_event(job_id, cur, status, "finish")
        else:
            logger.warning(
                "finish_job %s %s->%s skipped: concurrent status change (0 rows updated)",
                job_id,
                cur,
                status,
            )
    ...
    def accrue_cost(self, job_id: str, amount: float) -> None:
        """Add ``amount`` (USD) to a job's total_cost (NULL-safe accumulation across attempts).

        Uses COALESCE so a job whose total_cost starts as NULL is treated as 0
        on the first accrual. Successive calls sum into the running total,
        enabling per-attempt cost to be recorded independently and aggregated
        correctly across retries.

        Args:
            job_id: The job id whose total_cost to increment.
            amount: Cost in USD to add (must be non-negative).
        """
        self.conn.execute(
            "UPDATE jobs SET total_cost = COALESCE(total_cost, 0) + ? WHERE id=?",
            (amount, job_id),
        )
    ...
    def requeue(
        ...
        """Put a failed/dead/cancelled job back in the queue (manual or auto retry).
        ...
        the same UPDATE statement (single round-trip). When ``next_eligible_at`` is
        given, the job is not claimable until that UTC ISO timestamp; pass ``None``
        (the default) for immediate eligibility (e.g. manual CLI retry).

        Guards the transition via LEGAL_TRANSITIONS before executing the UPDATE.
        Idempotent: if the job is already 'pending' the call is a silent no-op.
        Atomic: UPDATE WHERE status IN ('failed','dead','cancelled') is
        belt-and-suspenders against a concurrent status change between the read
        and the write.
        ...
        Args:
            ...
            next_eligible_at: Optional UTC ISO 8601 timestamp before which the job
                must not be claimed. None means immediately eligible.

        Raises:
            IllegalTransitionError: If job does not exist, or current → pending
                is not a legal FSM transition (e.g. done → pending is forbidden).
        """
        row = self.get_job(job_id)
        if row is None:
            raise IllegalTransitionError(f"no such job: {job_id}")
        ...
        if cur == "pending":
            return  # idempotent: already at target
        assert_transition(cur, "pending", reason="requeue")
        if next_provider is not None:
            cursor = self.conn.execute(
                "UPDATE jobs SET status='pending', error_type=NULL, finished_at=NULL,"
                " worker_id=NULL, lease_until=NULL, provider=?, next_eligible_at=?"
                " WHERE id=? AND status IN ('failed','dead','cancelled')",
                (next_provider, next_eligible_at, job_id),
            )
        else:
            cursor = self.conn.execute(
                "UPDATE jobs SET status='pending', error_type=NULL, finished_at=NULL,"
                " worker_id=NULL, lease_until=NULL, next_eligible_at=?"
                " WHERE id=? AND status IN ('failed','dead','cancelled')",
                (next_eligible_at, job_id),
            )
        # Only record the audit event if a row actually transitioned (see finish_job).
        if cursor.rowcount == 1:
            self._record_job_event(job_id, cur, "pending", "requeue")
        else:
            logger.warning(
                "requeue %s %s->pending skipped: concurrent status change (0 rows updated)",
                job_id,
                cur,
            )
    ...
    def mark_dead(self, job_id: str) -> None:
        ...
        Guards the transition via LEGAL_TRANSITIONS before executing the UPDATE.
        Idempotent: if the job is already 'dead' the call is a silent no-op.
        Atomic: UPDATE WHERE status='failed' is belt-and-suspenders against a
        concurrent status change between the read and the write.

        Args:
            job_id: The job id to mark as dead.

        Raises:
            IllegalTransitionError: If job does not exist, or current → dead
                is not a legal FSM transition.
        """
        row = self.get_job(job_id)
        if row is None:
        ...
        if cur == "dead":
            return  # idempotent: already at target
        assert_transition(cur, "dead", reason="mark_dead")
        cursor = self.conn.execute(
            "UPDATE jobs SET status='dead' WHERE id=? AND status='failed'",
            (job_id,),
        )
        # Only record the audit event if a row actually transitioned (see finish_job).
        if cursor.rowcount == 1:
            self._record_job_event(job_id, cur, "dead", "mark_dead")
        else:
            logger.warning(
                "mark_dead %s %s->dead skipped: concurrent status change (0 rows updated)",
                job_id,
                cur,
            )
    ...
    def set_schedule_run_job_id(
        self, schedule_id: str, scheduled_for: str, job_id: str
    ) -> None:
