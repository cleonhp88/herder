...
class LocalRuntimeSpec(BaseModel):
    """Runtime spec for local subprocess execution.

    No additional fields — local is the implicit default.
    """
...
class DockerRuntimeSpec(BaseModel):
    """Runtime spec for Docker container execution.

    Args:
        type: Discriminator field, always "docker".
        image: Docker image name (required).
        network: Docker network mode; defaults to "none" for isolation.
        extra_args: Additional flags passed to ``docker run``.
    """
...
class SSHRuntimeSpec(BaseModel):
    """Runtime spec for remote SSH execution.

    Args:
        type: Discriminator field, always "ssh".
        host: SSH destination, e.g. ``user@host``.
        remote_root: Working directory on the remote host.
        ssh_opts: Additional flags passed to ``ssh``.
        allow_remote_secrets: Permit forwarding secret env vars to remote host.
    """
...
RuntimeSpec = Annotated[
    Union[LocalRuntimeSpec, DockerRuntimeSpec, SSHRuntimeSpec],
...
class Provider(BaseModel):
    ...
    # Providers sharing a concurrency_group share ONE worker semaphore, so models on the
    # same physical host (which holds only one model in memory at a time) are serialised
    # instead of thrashing. None = the provider is its own group (isolated, prior behaviour).
    ...
    # Runtime selection: name of a runtime declared in Config.runtimes,
    # or None to fall back to project default → "local".
...
class Project(BaseModel):
    ...
    # Default runtime for all jobs in this project; None falls back to "local".
...
class Worker(BaseModel):
    ...
    timezone: str = "UTC"
    retry_backoff_base_seconds: int = Field(default=0, ge=0)
    retry_backoff_max_seconds: int = Field(default=300, gt=0)
...
class Config(BaseModel):
    ...
    def _runtime_known(self, name: str | None) -> bool:
        """Return True if name is a declared runtime name or the implicit 'local'.

        Args:
            name: Runtime name to check, or None (always valid — falls back).

        Returns:
            True when name is None, "local", or in self.runtimes.
        """
        return name is None or name == "local" or name in self.runtimes
    ...
    def validate_refs(self) -> None:
        ...
        for pname, prov in self.providers.items():
            ...
            if prov.type == "ollama" and (not prov.base_url or not prov.model):
                raise ConfigError(
                    f"provider '{pname}' (ollama) needs base_url and model"
                )
        ...
        for sch in self.schedules:
            ...
            if sch.project not in self.projects:
                raise ConfigError(
                    f"schedule '{sch.id}' references unknown project '{sch.project}'"
                )
            if sch.role not in self.roles:
                raise ConfigError(
                    f"schedule '{sch.id}' references unknown role '{sch.role}'"
                )

        # Validate runtime references on providers and projects.
        for pname, prov in self.providers.items():
            if not self._runtime_known(prov.runtime):
                raise ConfigError(
                    f"provider '{pname}' references unknown runtime '{prov.runtime}'"
                )
        for prjname, proj in self.projects.items():
            if not self._runtime_known(proj.default_runtime):
                raise ConfigError(
                    f"project '{prjname}' references unknown runtime "
                    f"'{proj.default_runtime}'"
                )
