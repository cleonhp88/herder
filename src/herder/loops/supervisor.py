...
def execute_job(cfg: Config, store: Store, job, worker_id: str) -> str:
    ...
    project = cfg.projects.get(job["project"]) if job["project"] else None
    from herder.runtimes.resolve import resolve_runtime
    runtime = resolve_runtime(cfg, job, provider, project)
    ...
    res = execute(
        ...
        runtime=runtime,
        perms=perms,
        secret_keys=allow,  # resolved secret allow list — NOT the base env keys
    )
    ...
    # Accrue per-attempt cost into jobs.total_cost (NULL-safe, skipped when rate unknown)
    attempt_cost = estimate_cost(res.usage, provider.cost_key)
    if attempt_cost is not None:
        store.accrue_cost(job["id"], attempt_cost)
